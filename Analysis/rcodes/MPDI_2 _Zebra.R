...
## Rcodes to create the combined_surrogate_frequency.rds file

library(dplyr)

# Set your base directory
base_dir <- "./../analysis/SimulationOutsvazebra"

# Define the scenario folders
scenario2_dir <- file.path(base_dir, "ModelSize_1_nGene_2000_B_100_alpha0_0.05_ideal_FALSE")
scenario1_dir <- file.path(base_dir, "ModelSize_1_nGene_2000_B_100_alpha0_0.05_ideal_TRUE")

# Function to extract ncol from specific components of each .rds file
extract_surrogate_counts <- function(folder_path, scenario_name) {
  ...
  for (file in rds_files) {
    ...
    # Extract ncol for each method
    if ("FSRsvacov" %in% names(data)) {
      fsr_count <- ncol(data$FSRsvacov)
    } else {
      fsr_count <- NA
    }
    
    if ("svacov0" %in% names(data)) {
      sva0_count <- ncol(data$svacov0)
    } else {
      sva0_count <- NA
    }
    
    if ("svacovall" %in% names(data)) {
      svaall_count <- ncol(data$svacovall)
    } else {
      svaall_count <- NA
    }
    ...
    file_results <- data.frame(
      Method = c("FSR_sva", "SVA0", "SVAall_FSR"),
      Scenario = scenario_name,
      Num_Surrogates = c(fsr_count, sva0_count, svaall_count),
      File = basename(file),
...
# Extract data from both scenarios
scenario1_data <- extract_surrogate_counts(scenario1_dir, "Scenario_1")
scenario2_data <- extract_surrogate_counts(scenario2_dir, "Scenario_2")

# Combine both scenarios
combined_data <- bind_rows(scenario1_data, scenario2_data)

# Display the results
cat("=== SCENARIO 1 DATA ===\n")
print(head(scenario1_data, 15))

cat("\n=== SCENARIO 2 DATA ===\n")
print(head(scenario2_data, 15))

cat("\n=== COMBINED DATA ===\n")
print(head(combined_data, 20))

# Create frequency tables
freq_table <- combined_data %>%
  group_by(Method, Scenario, Num_Surrogates) %>%
  summarise(Frequency = n(), .groups = "drop") %>%
  arrange(Method, Scenario, Num_Surrogates)

cat("\n=== FREQUENCY TABLE ===\n")
print(freq_table)

# Create the final combined frequency table (wide format)
...
cat("\n=== FINAL COMBINED FREQUENCY TABLE ===\n")
print(final_table)

# Save the results
saveRDS(final_table, file.path(base_dir, "combined_surrogate_frequency.rds"))
cat("\nSaved to:", file.path(base_dir, "combined_surrogate_frequency.rds"))
